<template>
    <div>
        <h1 class="text-2xl font-bold text-gray-800 mb-6">Edit Fund</h1>

        <div v-if="loading" class="text-gray-500">Loading...</div>

        <form v-else @submit.prevent="submit" class="bg-white shadow rounded-lg p-6 max-w-2xl">
            <div class="space-y-4">
                <div>
...
async function submit() {
    ...
    try {
        ...
        await api.put(`/funds/${route.params.id}`, payload);
        router.push('/funds');
    } catch (e) {
...
onMounted(async () => {
    const [fundRes, managersRes, companiesRes] = await Promise.all([
        api.get(`/funds/${route.params.id}`),
